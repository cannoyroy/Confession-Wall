...
"func ReportPost(c *gin.Context) {
	...
	if err := db.Where("ReportID = ?", req.ReportID && "ReporterID = ?", req.ReporterID && "PostID = ?", req.PostID).First(&submit).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			// 继续举报
		} else {
			c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": "Internal server error"})
		...
		// 已举报过
		c.JSON(http.StatusOK, gin.H{"code": 200501, "data": nil, "msg": "已举报过"})
		return
	}
...
func ReportGet(c *gin.Context){
	...
	if err := db.Find(&reportG).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": "Internal server error"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"code": 200, "data": gin.H{"reportGet": reportG}, "msg": "success"})
}

func ReportTrash(c *gin.Context){
	var reportTrashG models.Trash
	if err := db.Find(&reportTrashG).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": "Internal server error"})
		return
	}
